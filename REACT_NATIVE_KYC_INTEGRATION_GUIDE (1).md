...
- Each base64 field is capped at ~14 MB by identity-proofing, while the BFF
  caps the **entire JSON request at 20 MB**. Keep the combined front + back +
  selfie payload safely below 20 MB. Compress via `expo-image-manipulator`
...
(`verified` | `failed` | `pending`) and the persisted extraction fields.
`matchScore` remains normalized to `0..1` (for example, `0.87`, not `87`).
Document numbers remain encrypted at rest and masked in every public response;
`extractedDocumentNumber` therefore returns the real OCR-derived number in
masked form. Image URL fields are present but remain `null` until private object
storage and short-lived signed-read URLs are deployed.
...
```json
{
  ...
  "extractedDocumentNumber": "••••4567",
  "dateOfExpiry": "2028-01-14",
  "nationality": "USA",
...
export interface IdentityDocument {
  ...
  extractedDocumentNumber: string | null; // masked
...
export interface VerificationSession {
  ...
  portraitImageUrl: string | null;
  documentImageUrl: string | null;
  matchScore: number | null; // normalized 0..1
  decisionId?: string;
...
export function useDocumentVerification() {
  ...
  const run = useCallback(async (input: {
    ...
    try {
      setPhase("creating");
      const session = await createVerificationSession(input.documentId, input.requestId);
      setPhase("verifying");
      const result = await verifyDocumentSession({
        sessionId: session.sessionId,
        frontImageBase64: input.frontBase64,
        backImageBase64: input.backBase64,
        selfieImageBase64: input.selfieBase64,
      });
      if (result.status === "completed") { setPhase("done"); return result; }
      const polled = await pollVerificationSession(result.sessionId);
      if (polled.status !== "completed") throw new Error("Document verification is still pending");
      setPhase("done");
      return polled;
    } catch (e) { setError(apiErrorMessage(e)); setPhase("failed"); throw e; }
  }, []);
}
```
...
```json
{
  ...
  "extractedName": "DOE JOHN", "extractedDob": "1990-01-15",
  "extractedDocumentNumber": "••••4567", "dateOfExpiry": "2028-01-14",
  "nationality": "USA", "issuingState": "California", "matchScore": 0.87,
  "portraitImageUrl": null, "documentImageUrl": null,
  "document": { "id": "…", "status": "verified", "number": "••••4567" }
}
```
...
- [ ] Any image > 14 MB, or combined JSON body > 20 MB → 413 → compress + retry.
